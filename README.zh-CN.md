...
> 当前正在以 `v0.7.0` 预览基线准备 `v1.0.0` 稳定开源版。Maven Central 是 v1.0 的目标安装路径，但必须等 release closure 确认发布后才能视为可用。
...
├── reliable-task-spring-boot-starter  # Worker-only Spring Boot 自动装配
├── reliable-task-admin-spring-boot-starter # Admin REST 自动装配和 Web 依赖
...
| `reliable-task-admin` | 管理 API、任务运维接口、指标收集 |
| `reliable-task-spring-boot-starter` | Worker-only 自动装配和配置属性 |
| `reliable-task-admin-spring-boot-starter` | Admin REST 自动装配和 Web 依赖，需显式引入 |
| `reliable-task-demo` | 可运行示例工程 |

## 架构总览

ReliableTask 的核心思路是把“业务事务提交后必须可靠执行的异步动作”先持久化为任务，再由 Worker 以可重试、可补偿、可观测的方式执行。它不依赖外部 MQ，当前实现以 MySQL 作为任务状态和执行日志的事实来源。
...
如果应用已经使用迁移工具，请只选择一种初始化方式：

- 原始 SQL：`reliable-task-store/src/main/resources/db/schema.sql`
- Flyway：`classpath:db/migration/V1__init_reliable_task_schema.sql`
- Liquibase：`classpath:db/changelog/db.changelog-master.yaml`

不要在同一个数据库中混用这些初始化方式，因为它们会创建同一组 ReliableTask 表和索引。
...
保持 Demo 后端运行在 `http://localhost:8080`，另开终端启动独立控制台预览版：
...
访问 `http://localhost:5173`。Vite dev server 默认把 `/api/reliable-task` 代理到 Demo 后端；如果后端地址不同，可在 `reliable-task-console/.env.local` 中覆盖：

```bash
VITE_RELIABLE_TASK_PROXY_TARGET=http://localhost:8080
```

Demo 配置只用于本地体验。由于 `application-example.yml` 默认未启用审计和授权，控制台会展示只读排障视图，并根据后端 capabilities 禁用写操作按钮。生产写操作仍必须具备认证、授权、审计、`X-Confirm-Operation: true`、网络访问控制和操作人追踪。

## 文档导航

新开发者可以优先阅读 `docs/getstart` 下的入门文档：

| 需求 | 入口 |
| --- | --- |
| 了解 ReliableTask 是什么 | [项目总览](docs/getstart/00-project-overview.md) |
| 理解运行时架构 | [架构说明](docs/getstart/01-architecture.md) |
| 理解 Maven 模块和 console 边界 | [模块依赖图](docs/getstart/02-module-map.md) |
| 跟踪投递、执行、重试、恢复和 Admin 写保护 | [核心流程](docs/getstart/03-core-flows.md) |
| 查看表结构、状态和 schema 归属 | [领域模型与数据模型](docs/getstart/04-domain-model.md) |
| 对接 Admin API 或 console contract | [API 设计](docs/getstart/05-api-design.md) |
| 安全部署 demo、Admin 和 console | [部署说明](docs/getstart/06-deployment.md) |
| 扩展 handler、重试、codec、auth、audit 或 store SPI | [扩展点说明](docs/getstart/07-extension-guide.md) |
| 本地启动和验证 | [本地开发](docs/getstart/08-local-development.md) |
| 排查积压、DEAD、重试风暴或验证阻塞 | [常见问题排查](docs/getstart/09-troubleshooting.md) |
| 快速查看高频问题 | [FAQ](docs/getstart/10-faq.md) |

## 接入方式

对 `v1.0.0` 用户，Maven Central 是 release closure 确认发布后的优先安装路径。仅运行 Worker 时引入基础 Spring Boot starter：

如果正在规划从 v0.x 升级到 v1.0，请先阅读 [v1.0 升级指南](docs/migration/v1.0-upgrade-guide.md)。该文档覆盖兼容保留 API、已废弃但保留的 `TaskSerializer`、`TaskStore` 迁移建议、schema 初始化三选一、Maven Central 发布状态、验证命令和回滚注意事项。

需要查找可运行示例时，请参考 [v1.0 示例矩阵](docs/v1.0/RELIABLE_TASK_V10_EXAMPLE_MATRIX.md)，其中索引了成功、重试、不可重试失败、显式幂等键、死信、Admin 安全、监控、控制台和迁移示例。
...
如果应用还需要 Admin REST API，需要额外显式引入 Admin starter：
...
在 `v1.0.0` 出现在 Maven Central 之前，请通过源码构建、本地安装或私有 Maven 仓库引用：

```bash
mvn -B -DskipTests install
```

如果在 v1.0 发布前消费预览构建，请把依赖版本替换为本地安装或私有仓库中的版本，例如 `0.7.0`。

控制台预览版位于独立前端工程 `reliable-task-console`。它不是 worker-only 应用的依赖，也不会被打包进任一 Spring Boot starter。部署时应将构建后的 `dist/` 静态资源放在受保护的内部运维入口后面，并通过反向代理把 `/api/reliable-task` 转发到已启用 Admin starter 的业务应用。预览范围、Admin 安全边界和非目标见 [Console and Admin roadmap](docs/console-admin-roadmap.md)。
...
可运行 Demo 会为了本地体验显式启用 Admin API。这些 Demo 配置不是生产默认值。即使配置了写开关，Admin 写接口仍会在服务端要求写开关、权限检查、审计和确认 header 同时满足后才会执行真实写入。Admin REST API 还需要引入 `reliable-task-admin-spring-boot-starter`，仅设置 `reliable-task.admin.enabled=true` 不会让 worker-only starter 创建 Admin controller。
...
| `reliable-task.executor.mode` | `platform` | 执行器模式；`platform` 使用传统有界平台线程池，`virtual` 使用 JDK 21 虚拟线程且需显式开启 |
| `reliable-task.executor.default-core-size` | `4` | platform 线程池默认核心线程数；virtual 模式下保留为兼容配置 |
| `reliable-task.executor.default-max-size` | `16` | platform 线程池默认最大线程数；virtual 模式下作为最大并发 Handler 任务数 |
| `reliable-task.executor.default-queue-capacity` | `100` | platform 线程池默认队列容量；virtual 模式下仅保留兼容，不作为虚拟线程队列 |
| `reliable-task.executor.keep-alive-seconds` | `60` | platform 线程空闲存活时间，单位秒 |
...
| `reliable-task.admin.write-enabled` | `false` | Admin 写接口开关，包括重试、终止、重新入队、更新 payload 和批量操作；开启后仍必须满足权限、审计和确认检查 |
...
| `reliable-task.admin.auth.enabled` | `true` | Admin 权限 SPI 开关；Admin API 显式注册后默认启用权限检查，写操作必需 |
| `reliable-task.admin.audit.enabled` | `false` | Admin 操作审计和审计日志查询开关，写操作必需 |
| `reliable-task.admin.batch.enabled` | `false` | 批量运维 API 开关，关闭时接口返回 404 |
| `reliable-task.admin.console.payload-plaintext-enabled` | `false` | 是否允许 console-safe 详情响应包含 payload 明文，默认关闭 |
| `reliable-task.admin.console.payload-preview-length` | `512` | console-safe payload preview 最大长度 |
| `reliable-task.admin.console.write-confirmation-required` | `true` | Admin 写操作是否要求 `X-Confirm-Operation: true` |

`admin.query.*` 默认值只作用于 v0.5 新增的运维查询，例如最近失败、慢任务、失败聚合和 timeline 相关列表视图。现有 `/tasks` 和 `/audit-logs` 列表接口保持兼容过滤行为，不会被自动追加 24 小时时间窗口。

以下兼容保留配置在 v1.0 线仍可绑定，但不会改变运行行为：`reliable-task.serializer.type` 不会切换序列化器，如需自定义请提供 `TaskPayloadSerializer` Bean；`reliable-task.store.table-prefix` 不会改变 MyBatis 表名；`reliable-task.admin.port` 和 `reliable-task.admin.context-path` 不会创建独立管理端口，也不会改变当前 `/api/reliable-task` 映射。

虚拟线程执行模式只在 Java 21 基线下可用，必须通过 `reliable-task.executor.mode=virtual` 显式开启。默认 `platform` 模式保持既有有界线程池行为。virtual 模式下，`max-size` 仍作为背压和容量上限；`core-size` 与 `queue-capacity` 保留可绑定以兼容已有配置，但不会创建虚拟线程队列。

面向 v0.6 的存储扩展应依赖所需的最窄 Store SPI：投递、抢占和状态更新使用 `TaskCommandStore`；Admin、metrics、alert 只读查询使用 `TaskQueryStore`；心跳和 Admin 写操作等运维能力使用 `TaskOperationsStore`。旧 `TaskStore` 继续作为兼容门面保留，并继承这些窄接口。

面向 v0.6 的新接入代码中，`TaskSerializer` 仅作为已废弃的源码兼容 SPI 保留。自定义 payload 序列化请提供 `TaskPayloadSerializer` Bean；需要上下文感知的 codec 能力将预留给后续 `TaskPayloadCodec` 路径。

如果需要在 v0.6 中基于任务上下文自定义 payload 处理，可以提供 `TaskPayloadCodec` Bean。未提供 codec Bean 时，starter 会把当前生效的 `TaskPayloadSerializer` Bean 自动适配为 codec：
...
任务投递还会使用 `TaskTraceIdGenerator`：默认实现优先复用当前 `TraceContext`，如果当前线程没有 traceId，则写入一个 `rt-` 前缀且符合现有 `VARCHAR(64)` 长度约束的 traceId。如需替换生成策略，请提供自定义 `TaskTraceIdGenerator` Bean。

任务执行现在使用有序 `TaskInterceptor` 链。可以注册多个 `TaskInterceptor` Bean，并在顺序敏感时使用 Spring `@Order`；内置 trace interceptor 保持与 v0.5 兼容的 trace 清理行为。

Handler 注册会使用 `TaskNameResolver`，并在 registry 中记录 `TaskHandlerMetadata`。默认 resolver 保留现有规则：`@TaskHandler` 与 `getTaskType()` 同时存在时必须一致；自定义 resolver 可以替换命名策略，但重复 taskType 检测仍由 registry 执行。
...
- Admin 安全：Admin API 应只暴露在内部运维网络；生产保持 `reliable-task.admin.auth.enabled=true`，默认关闭写接口；如需开启，必须接入认证、授权、审计、`X-Confirm-Operation: true`、网络访问控制和操作人追踪。
...
- Admin REST API 默认关闭。仅在本地 Demo 或受保护的内部运维场景启用 `reliable-task.admin.enabled=true`；写操作要求 `reliable-task.admin.write-enabled=true`、`reliable-task.admin.auth.enabled=true`、`reliable-task.admin.audit.enabled=true`，并在启用确认时提供 `X-Confirm-Operation: true`；生产环境还必须完成认证、授权、监控和网络访问控制。
...
不建议直接暴露。Admin REST API 默认关闭；如需启用 `reliable-task.admin.enabled=true`，生产环境应保持权限检查开启。如还需启用 `reliable-task.admin.write-enabled=true`，服务端默认还会要求审计和 `X-Confirm-Operation: true`，部署侧必须接入认证、授权、网络访问控制、监控和操作权限限制，尤其要保护重试、终止、重新入队、更新 payload 等写操作。

### v1.0 是否已经发布到 Maven Central？

需要等 release closure 确认后才视为发布完成。Maven Central 是 v1.0 的目标安装路径；在 `v1.0.0` 可见之前，请继续使用源码构建、本地安装或私有 Maven 仓库。
...
`0.x` 阶段属于首次开源预览期，API 和数据库 schema 仍可能调整。所有新增、修复、安全变更和破坏性变化都应记录在 [CHANGELOG.md](CHANGELOG.md)，发布前请优先阅读对应版本说明。v1.0 准备阶段请同时阅读 [升级指南](docs/migration/v1.0-upgrade-guide.md)。

## Roadmap

- `v1.0.0`：稳定 API/SPI、schema 初始化策略、release workflow、升级指南、测试矩阵、示例矩阵和 readiness report。
- Console/Admin：保持 console 独立于 Java starter，增强只读诊断，并继续让写操作受服务端 auth、audit 和 confirmation gate 保护。详见 [Console and Admin roadmap](docs/console-admin-roadmap.md)。
- 运维：持续对齐监控、runbook、Prometheus 示例与真实 Admin API 和 Micrometer 指标面。详见 [监控指南](docs/operations/reliable-task-monitoring.md) 和 [生产 Runbook](docs/operations/reliable-task-runbook.md)。

## 版本与发布

- 版本号遵循 SemVer。
- Git Tag 使用 `vX.Y.Z`，例如 `v0.7.0`。
...
- 最新已完成预览发布版本为 `v0.7.0`。
- `v1.0.0` 稳定版正在准备 Maven Central 发布、release workflow、升级指南、测试矩阵、示例矩阵和 readiness report。
...
欢迎通过 Issue 和 Pull Request 参与改进。提交前请阅读 [CONTRIBUTING.md](CONTRIBUTING.md) 和 [community onboarding guide](docs/community-onboarding.md)，并在 PR 中说明变更范围、测试结果、安全影响和兼容性影响。
