...
> 当前预览版本为 `v0.7.0`。生产使用前请完成数据库备份、Admin 鉴权、监控告警和容量评估。
...
保持 Demo 后端运行在 `http://localhost:8080`，另开终端启动独立 v0.7 控制台：
...
当前 `0.7.0` 预览阶段默认通过源码构建后在本地或私有仓库引用，尚未发布到 Maven Central。仅运行 Worker 时引入基础 Spring Boot starter：
...
v0.7 控制台预览版位于独立前端工程 `reliable-task-console`。它不是 worker-only 应用的依赖，也不会被打包进任一 Spring Boot starter。部署时应将构建后的 `dist/` 静态资源放在受保护的内部运维入口后面，并通过反向代理把 `/api/reliable-task` 转发到已启用 Admin starter 的业务应用。
...
可运行 Demo 会为了本地体验显式启用 Admin API。这些 Demo 配置不是生产默认值。从 v0.7 开始，即使配置了写开关，Admin 写接口仍会在服务端要求写开关、权限检查、审计和确认 header 同时满足后才会执行真实写入。Admin REST API 还需要引入 `reliable-task-admin-spring-boot-starter`，仅设置 `reliable-task.admin.enabled=true` 不会让 worker-only starter 创建 Admin controller。
...
以下兼容保留配置在 `0.7.0` 中仍可绑定，但不会改变运行行为：`reliable-task.serializer.type` 不会切换序列化器，如需自定义请提供 `TaskPayloadSerializer` Bean；`reliable-task.store.table-prefix` 不会改变 MyBatis 表名；`reliable-task.admin.port` 和 `reliable-task.admin.context-path` 不会创建独立管理端口，也不会改变当前 `/api/reliable-task` 映射。
...
还没有。`0.7.0` 预览阶段默认通过源码构建、本地安装或私有 Maven 仓库引用。Maven Central 发布状态仍是 TODO。
